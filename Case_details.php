...
// update date time function 
    ...
    public function genratedata(){
  
       $this->load->model('admin/Payment_model');
        
  
       if(isset($_POST['paymentval']) && !empty($_POST['paymentval'])){
       
        $data = array(
            
            'case_id'      =>$_POST['case_id'],
            'amount' => $_POST['paymentval']
            
         );
   
             $case_data = $this->Case_details_model->find($_POST['case_id']);
                          
             // update payment table

             $w=array(
                'id' => $case_data->payment_id,
                'amount' => intval($_POST['paymentval']),
                'seen' => 0,
                'updateAt' => date("Y-m-d H:i:s"),
                'updatedBy' => "Admin",
                'pay_link' => base_url()."dpay/pay?u=".base64_encode($case_data->client_id)."&ci=".base64_encode($case_data->id)."&amt=".base64_encode($_POST['paymentval'])."&p=".base64_encode($case_data->payment_id),
             );

             $this->payment_model->save($w);
   ...
   // generate payment data
    ...
    // freemeetdata payment data
    public function freemeetdata(){
        ...
        if(isset($_POST['action']) && $_POST['action'] == 'paymentStatusUpdate' ){
        
         $data = array(
             
             'case_id'      =>$_POST['case_id'],
             'commentval'      =>$_POST['commentval'],
             'freemeetval'      =>$_POST['freemeetval'],
             'amount' => '0'    
              ...
              $freemeet_data = $this->Case_details_model->find($_POST['case_id']);
                // get payment logs
              $oldLog = empty($payment->pay_logs)?array():json_decode($payment->pay_logs,true);
                $oldLog[] = array(
                    'date' => date("Y-m-d h:i a"),
                    'for' => "Admin changed payment status",
                    'by' => "Admin",
                );
              ...
              $w=array(
                 'id' => $freemeet_data->payment_id,
                 'amount' => intval('0'),
                 'commentval'=>$_POST['commentval'],
                 'payment_status'=> $_POST['payStatus'],
                 'payment_method'=> ($_POST['freemeetval'] == 'Free')?$_POST['freemeetval']:$_POST['commentval'] ,
                 'updateAt' => date("Y-m-d H:i:s"),
                 'updatedBy' => "Admin",
                 'pay_logs' => json_encode($oldLog),
              );

              if($_POST['freemeetval'] != 'Free') {
                unset($w['amount']);
              }
    ...
    // generate payment data
     ...
     public function update_paymentstatus(){
      ...
      //Direct form update
        ...
        if(isset($_POST['checkform']) && !empty($_POST['checkform'])){
          // pre($_POST);
          // exit();
            $status12 = $this->input->post('selectstatus')
            $case_data = $this->Case_details_model->find($_POST['case_id']);


          $data = array(
              'id'=>$case_data->payment_id,
              'payment_status' => $_POST['selectstatus'],
          );
          $this->payment_model->save($data);
